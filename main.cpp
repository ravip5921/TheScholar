...
namespace LogIn
{
    ...
    Coord_Rect userNameD(-5, 1, 9, 1.9);
...
void mousePressed(int button, int state, int x, int y)
{
    ...
    if (PAGE == WELCOME_P)
    {
        if(activePage.buttonPressed(welcome::signupButton))
        {
            PAGE = SIGNUP_P;
        }
        else if(activePage.buttonPressed(welcome::loginButton))
        {
            PAGE = LOGIN_P;
        }
    }
    ...
    {
        if(activePage.buttonPressed(LogIn::logInButton))
        {
            // log the user in
           // PAGE = SIGNUP_P;
            //std::cout << "HEY\n";
        }
        else if(activePage.buttonPressed(LogIn::toSignup))
        {
            PAGE=SIGNUP_P;
        }
    }
    ...
    {
        if(activePage.buttonPressed(SignUp::signUpButton))
        {
            PAGE = LOGIN_P;
            //std::cout << "signed In\n";
        }
